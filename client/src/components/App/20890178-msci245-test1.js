...
const App = () => {
  ...
  const [productState,setProductState] = React.useState(products);
  const [discountedProducts, setDiscountedProducts] = React.useState(true);
  ...
  //DiscountCheckBox
  const handleDiscountedProducts = (checked,item) => {
    setDiscountedProducts(item.discount)
  }
...
const List = ({ list, inStock, discountedProducts, onDiscountedPrices}) => {
  return (
    <ul>
      {list.map((item) => {
        return (
              ...
              onDiscountedPrices={onDiscountedPrices}
              />
        );
      })}
    </ul>
...
const Item = ({ item, inStock, discountedProducts }) => {
